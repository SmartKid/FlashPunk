package net.flashpunk.graphics 
{
	...
	public class Emitter extends Graphic
	{
		...
		/** @private Renders the particles to Stage3D. */
		override public function renderStage3D(context:Context3D, point:Point, camera:Point):void
		{
			if (!_particle || !context) return;
			
			if (_screenWidth != FP.width || _screenHeight = FP.height)
			{
				_screenWidth = FP.width;
				_screenHeight = FP.height;
				if (_vertexBuffer) _vertexBuffer.dispose();
				_vertexBuffer = context.createVertexBuffer(4, 5); // 4 vertices of 5 coordinates each
				_vertexBuffer.uploadFromVector(vertices, 0, 4); // 0 offset, 4 vertices
			}
			
			var buffer:BitmapData(_screenWidth, _screenHeight, true, 0x00000000);
			buffer.lock();
			render(buffer, point, camera);
			buffer.unlock();
			...
			var vertices:Vector.<Number> = Vector.<Number>([
				0,            0,             0, 0, 0, // x, y, x, u, v
				_screenWidth, 0,             0, 1, 0,
				0,            _screenHeight, 0, 0, 1,
				_screenWidth, _screenHeight, 0, 1, 1
			]);
			
			context.setVertexBufferAt(0, _vertexBuffer, 0, Context3DVertexBufferFormat.FLOAT_3); // assigns XYZ coordinates to register 'va0'
			context.setVertexBufferAt(1, _vertexBuffer, 3, Context3DVertexBufferFormat.FLOAT_2); // assigns UV coordinates to register 'va1'
			
			texture = context.createTexture(_screenWidth, _screenHeight, Context3DTextureFormat.BRGA, false);
			texture.uploadFromBitmapData(buffer);
			context.setTextureAt(0, texture); // assigns texture to register 'fs0'
		...
		/** @private */ private var _screenWidth:uint = 0;
		/** @private */ private var _screenHeight:uint = 0;
		
		// Stage3D information.
		/** @private */ private static var _program:Program3D;
		/** @private */ private static var _vertexBuffer:VertexBuffer3D;
